...
class Player(Section):
    ...
    def setup(self):
        # resets the players paddle to the middle of the screen
    ...
    def on_draw(self):
        # draws the sections, info and score
        if self.name == 'Left':
            keys = 'W and S'
            start_x = self.left + 5
        else:
...
class Pong(View):
    ...
    def __init__(self):
        ...
        self.left_player: Player = Player(
            0, 0, SECTION_WIDTH, self.window.height, key_up=W,
            key_down=S, name='Left')
        self.right_player: Player = Player(
            self.window.width - SECTION_WIDTH, 0, SECTION_WIDTH,
